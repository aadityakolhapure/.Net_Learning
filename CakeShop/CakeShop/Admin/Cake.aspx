                        ...
                        if (Request.Form["addCake"] != null)
                        {
                            ...
                            if (Request.Files["imageFile"] != null && Request.Files["imageFile"].ContentLength > 0)
                            {
                                try
                                {
                                    ...
                                    string fileName = System.IO.Path.GetFileName(file.FileName); // original file name

                                    // Create directory if it doesn't exist
                                    string uploadDir = Server.MapPath("../Images/Cakes/");
                                    if (!System.IO.Directory.Exists(uploadDir))
                                    {
                                        System.IO.Directory.CreateDirectory(uploadDir);
                                    }

                                    string savePath = System.IO.Path.Combine(uploadDir, fileName);
                                ...
                                {
                                    Response.Redirect("~/Admin/Cake.aspx?msg=Error uploading image: " + ex.Message + "&type=error");
                                    return;
                                }
                            }
